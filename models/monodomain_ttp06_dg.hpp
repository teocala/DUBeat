...
public:
  /// Constructor.
  ExactSolution() : lifex::utils::FunctionDirichlet() {}

  /// Evaluate the exact solution in a point.
  virtual double value(const dealii::Point<lifex::dim> &p,
...
public:
  /// Constrcutor.
  BCNeumann() : lifex::Function<lifex::dim>() {}
...
public:
  /// Constructor.
  GradExactSolution() : lifex::Function<lifex::dim>() {}
...
template <class basis> class MonodomainTTP06DG : public ModelDG_t<basis> {
public:
  /// Constructor.
...
private:
  ...
  void declare_parameters(lifex::ParamHandler &params) const override;

  /// Override to parse additional parameters.
  void parse_parameters(lifex::ParamHandler &params) override;
...
template <class basis> void MonodomainTTP06DG<basis>::update_time() {
...
void MonodomainTTP06DG<basis>::declare_parameters(
  ...
  {
    params.declare_entry("Filename", "../meshes/slab_test_mesh.msh",
                         dealii::Patterns::Anything(), "File msh for the mesh");
    params.declare_entry("Scaling factor", "1", dealii::Patterns::Double(0),
                         "Scaling factor");
  }
  ...
  params.enter_subsection("Discontinuous Galerkin");
  {
    params.declare_entry(
        "Penalty coefficient", "1", dealii::Patterns::Double(-1, 1),
        "Penalty coefficient in the Discontinuous Galerkin formulation.");
    params.declare_entry(
        "Stability coefficient", "10", dealii::Patterns::Double(0),
  ...
  {
    params.declare_entry("Initial time", "0", dealii::Patterns::Double(0),
                         "Initial time.");
    params.declare_entry("Final time", "0.001", dealii::Patterns::Double(0),
                         "Final time.");
    params.declare_entry("Time step", "0.0001", dealii::Patterns::Double(0),
                         "Time step.");
    params.declare_entry("BDF order", "1", dealii::Patterns::Integer(1, 3),
                         "BDF order: 1, 2, 3.");
  }
  params.leave_subsection();
  ...
  {
    params.declare_entry("ChiM", "1e5", dealii::Patterns::Double(0),
                         "Surface-to-volume ratio of cells parameter.");
    params.declare_entry("Cm", "1e-2", dealii::Patterns::Double(0),
                         "Membrane capacity.");
    params.declare_entry("sigma_m_t", "0.3494e-4", dealii::Patterns::Double(),
                         "Transversal conductivity.");
    params.declare_entry("sigma_m_l", "0.7643e-4", dealii::Patterns::Double(),
                         "Longitudinal conductivity.");
    params.declare_entry("sigma_m_n", "0.1125e-4", dealii::Patterns::Double(),
                         "Normal conductivity.");
    params.declare_entry("f0", default_f0, dealii::Patterns::Anything(),
                         "f0 fibers direction.");
    params.declare_entry("n0", default_n0, dealii::Patterns::Anything(),
                         "n0 fibers direction.");
  }
  params.leave_subsection();
  ...
  {
    params.declare_entry("Compute", "true", dealii::Patterns::Bool(),
...
template <class basis>
void MonodomainTTP06DG<basis>::parse_parameters(lifex::ParamHandler &params) {
  // Parse input file.
  params.parse();
...
template <class basis> void MonodomainTTP06DG<basis>::run() {
  ...
  while (this->time < this->prm_time_final) {
    ...
    this->intermediate_error_print(this->solution_owned,
                                   this->solution_ex_owned, this->u_ex, "u");
    rhs_ionic.reinit(owned_dofs, this->mpi_comm);
  }

  this->compute_errors(this->solution_owned, this->solution_ex_owned,
...
template <class basis> void MonodomainTTP06DG<basis>::assemble_system() {
  ...
  std::vector<lifex::types::global_dof_index> dof_indices(this->dofs_per_cell);
    ...
    if (cell->is_locally_owned()) {
      ...
      for (const auto &edge : cell->face_indices()) {
        ...
        if (!cell->at_boundary(edge)) {
          S = this->assemble->local_SC(this->prm_stability_coeff, Sigma);
          std::tie(I, I_t) =
          ...
          SN = this->assemble->local_SN(this->prm_stability_coeff, Sigma);
          ...
          cell_rhs_edge = this->assemble->local_rhs_edge_neumann(this->g_n);
